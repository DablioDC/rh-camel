...
import br.com.unicoob.domain.template.DemissaoTemplate;
...
@Component
public class ChamadoDemissaoProcessor implements Processor {

    private static final Logger LOGGER = LoggerFactory.getLogger(ChamadoDemissaoProcessor.class);
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        DemissaoTemplate template = new DemissaoTemplate(funcionario);
        for (OperatorGroupDTO operadorGroup : template.getOperatorGroups()) {

            List<RhOperationLog> logs = logRepository.findAllByOperationAndNumcadAndNumempAndNumcpfAndOperatorGroup(OperationType.DEMISSAO, Integer.valueOf(funcionario.getMatricula()), funcionario.getCooperativa().getCooperativa(), Long.valueOf(funcionario.getCpf()), operadorGroup.getNome());
            if(!logs.isEmpty()){
                continue;
            }
            ...
            if (incident.getStatusCode().is2xxSuccessful() && incident.getBody()!= null) {
                ...
                RhOperationLog operationLog = new RhOperationLog(funcionario, OperationType.DEMISSAO, number, operadorGroup.getNome());
                logRepository.save(operationLog);
                LOGGER.info("SUCESSO: {} - Aberto o chamado de {} do colaborador {} para o grupo {}", number, OperationType.DEMISSAO, funcionario.getNomeCompleto(), operadorGroup.getNome());
            } else{
                LOGGER.warn("FALHA: Não foi possivel abrir o chamado de {} do colaborador {} para o grupo {}", OperationType.DEMISSAO, funcionario.getNomeCompleto(), operadorGroup.getNome());
